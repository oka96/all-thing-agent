...
async function sendChat(event) {
    ...
    try {
        const response = await api("/api/troubleshooting/chat", {
            ...
            body: JSON.stringify({
                message,
                customerRef: state.selectedCustomerRef,
                transferRef: explicitTransferRef(message) || state.lastTransferRef,
            }),
        });
        typing.remove();
    ...
    return `
        <div class="raw-table-wrap" tabindex="0" aria-label="Scrollable raw query results">
            <table class="raw-table">
                ...
                <tbody>
                    ${rows.map(row => `<tr>${columns.map(column => `<td>${row[column] == null ? "<em>NULL</em>" : escapeHtml(row[column])}</td>`).join("")}</tr>`).join("")}
                </tbody>
            </table>
        </div>`;
}
...
function updateChatContext() {
    const user = state.users.find(candidate => candidate.customerRef === state.selectedCustomerRef);
    const parts = [];
    if (user) parts.push(`${user.displayName} · ${user.customerRef}`);
    if (state.lastTransferRef) parts.push(state.lastTransferRef);
    elements.chatContext.textContent = parts.join(" · ") || "No customer context";
}
